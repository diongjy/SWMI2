...
ds=xr.open_dataset('./gefs/u850_'+str(forecast_date)+'_gfs_gefs_00z')
...
#print(df)
df.to_csv('./test/swmi_'+str(forecast_date)+'.csv',index=False, header=False)
...
df1=pd.read_csv('./test/swmi1.csv', header=None, parse_dates=True)
header=['Date','SWMI2']
df1.columns=header
df2=pd.read_csv('./test/swmi_'+str(forecast_date)+'.csv', header=None, parse_dates=True)
df2.columns=header
...
merged_df['15day']=merged_df['15day'].shift(-14)
...
plt.savefig('./test/GEFS_SWMI2.png', bbox_inches='tight')
df_new1=df_new.iloc[:,[0,1]]
df_new1.to_csv('./test/swmi.csv',index=False, header=False)
